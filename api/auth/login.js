// Combines the "start" and "callback" legs of Google Sign-In into one
// route/one serverless function -- Vercel's Hobby plan caps a deployment at
// 12 functions, so this file (and api/auth/me.js, api/push.js) merge what
// would otherwise be separate single-purpose endpoints. Google redirects
// back to this same URL with ?code=..., which is how the two legs are told
// apart below.
...
async function handleStart(req, res, redirectUri) {
...
async function handleCallback(req, res, redirectUri, code, state, error) {
...
module.exports = async (req, res) => {
  try {
    ...
    const redirectUri = `https://${req.headers.host}/api/auth/login`;

    if (code || error) {
      return await handleCallback(req, res, redirectUri, code, state, error);
    }
    return await handleStart(req, res, redirectUri);
  } catch (e) {
    res.statusCode = 500;
